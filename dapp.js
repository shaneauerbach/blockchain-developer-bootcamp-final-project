...
const wiAddress = '0x9096D27D1A24F53028F3A60943f31715620D2359'
...
wiGetProposedEntry.onclick = async () => {
    ...
    else if (entry.state == 4) {state = "Rejected"}
...
wiReject.onclick = async () => {
    ...
    if (checkID > 0 && checkState == 1 ){
        ...
        let tx = await whatIs.methods.rejectEntry(wiRejectWhatName).
        send({from: ethereum.selectedAddress});
